...
class TestEnroll:
    def test_new_profile_returns_ok(self, tmp_path):
        with _client(tmp_path) as client:
            resp = client.post("/enroll", content=_AUDIO, headers={"x-roomie-id": "leonie"})
        assert resp.status_code == 200
        assert resp.json()["ok"] is True

    def test_new_profile_written_to_disk(self, tmp_path):
        with _client(tmp_path) as client:
            client.post("/enroll", content=_AUDIO, headers={"x-roomie-id": "leonie"})
        assert (tmp_path / "disk" / "leonie.pt").exists()

    def test_new_profile_written_to_mem(self, tmp_path):
        with _client(tmp_path) as client:
            client.post("/enroll", content=_AUDIO, headers={"x-roomie-id": "leonie"})
        assert (tmp_path / "mem" / "leonie.pt").exists()
    ...
    def test_second_enroll_blends_embeddings(self, tmp_path):
        with _client(tmp_path) as client:
            ...
            resp = client.post("/enroll", content=_AUDIO, headers={"x-roomie-id": "leonie"})
        assert resp.status_code == 200
        saved = torch.load(str(tmp_path / "disk" / "leonie.pt"), map_location="cpu")
...
class TestIdentify:
    def test_no_profiles_returns_unknown(self, tmp_path):
        ...
        assert resp.json()["roomie_id"] == "unknown"
    ...
    def test_identifies_enrolled_roomie(self, tmp_path):
        with _client(tmp_path) as client:
            client.post("/enroll", content=_AUDIO, headers={"x-roomie-id": "leonie"})
            resp = client.post("/identify", content=_AUDIO)
        assert resp.json()["roomie_id"] == "leonie"
    ...
    def test_picks_best_of_multiple_profiles(self, tmp_path):
        with _client(tmp_path) as client:
            client.post("/enroll", content=_AUDIO, headers={"x-roomie-id": "leonie"})
            # Save a zero-embedding for rene directly into the shared dirs
            rene_emb = torch.zeros(192)
            torch.save(rene_emb, str(tmp_path / "disk" / "rene.pt"))
            torch.save(rene_emb, str(tmp_path / "mem" / "rene.pt"))
            # identify with ones-embedding → leonie (cos_sim=1) beats rene (cos_sim=0)
            resp = client.post("/identify", content=_AUDIO)
        assert resp.json()["roomie_id"] == "leonie"
...
class TestStartupProfileSync:
    def test_disk_profiles_copied_to_mem_on_startup(self, tmp_path):
        disk = tmp_path / "disk"
        disk.mkdir()
        torch.save(torch.ones(192), disk / "leonie.pt")
...
class TestConfigThresholds:
    def test_custom_thresholds_applied(self, tmp_path):
        with _client(tmp_path, unknown_threshold=0.9, uncertain_threshold=0.95) as client:
            client.post("/enroll", content=_AUDIO, headers={"x-roomie-id": "leonie"})
            # cos_sim == 1.0 which is >= 0.95 → not uncertain, returns leonie
            resp = client.post("/identify", content=_AUDIO)
        assert resp.json()["roomie_id"] == "leonie"
